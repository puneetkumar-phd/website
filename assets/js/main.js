...
(() => {
  ...
  updateFilterButtons.forEach(button => button.addEventListener('click', () => {
    updateFilterButtons.forEach(b => b.classList.remove('active'));
    button.classList.add('active');
    const filter = button.dataset.updateFilter;
    document.querySelectorAll('[data-update-type]').forEach(card => {
      const types = (card.dataset.updateType || '').split(/\s+/);
